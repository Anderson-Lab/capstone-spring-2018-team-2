...
mepsdsgn <- update(mepsdsgn,
                   ambexp15 = OBVEXP15 + OPTEXP15 + ERTEXP15,
                   hhexp15  = HHAEXP15 + HHNEXP15 + VISEXP15 + OTHEXP15)

# Use svyratio to calculate percentage distribution of spending by type of service:
pct_TOS = svyratio(~IPTEXP15 + ambexp15 + RXEXP15 + DVTEXP15 + hhexp15,
                   denominator = ~TOTEXP15,
                   design = mepsdsgn)

# Now do the same thing by age group (<65, 65+), using the `subset` function.
pct_TOS_lt65 = svyratio(~IPTEXP15 + ambexp15 + RXEXP15 + DVTEXP15 + hhexp15,
                        denominator = ~TOTEXP15,
                        design = subset(mepsdsgn,AGELAST < 65))

pct_TOS_ge65 = svyratio(~IPTEXP15 + ambexp15 + RXEXP15 + DVTEXP15 + hhexp15,
                        denominator = ~TOTEXP15,
                        design = subset(mepsdsgn,AGELAST >= 65))
