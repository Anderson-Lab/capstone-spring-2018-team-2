...
library(caret)
library(dplyr)
source("Join_Data_2014.R")
load('meta.rda')

# Get data
meps <- Join_MEPS_14()
meps.p <- meps[meps$PHOLDER == 1,]
mepsPublic14<-Public_Filter(meps.p)
mepsPrivate14<-Private_Filter(meps.p)

# Get vars
mepsPrivate14$age.cat <- Age.to.Cat(mepsPrivate14, 'AGE14X')
...
controls <- c('PHOLDER', 'CHBMIX42','BMINDX53','ADGENH42', 'age.cat', 'FAMINC14', 
...
factors <- c('IPDIS14', 'HOSPINSX', 'PLANMETL', 'HSAACCT', behaviors, 'PHOLDER',
             'CHBMIX42', 'ADGENH42','COBRA', 'OOPPREM', 'PREGNT31', 'PREGNT42', 'PREGNT53')

#Set target to binary
mepsPrivate14$IPDIS14[mepsPrivate14$IPDIS14>1] <- 1

#Coerce to fewer factors
mepsPrivate14$ANNDEDCT <- as.numeric(mepsPrivate14$ANNDEDCT)
for(variable in c(plan.dsn, behaviors)){
  mepsPrivate14[mepsPrivate14[,variable] < 0, variable] <- "Unknown"
}

for(factor in factors){
  mepsPrivate14[,factor] <- as.factor(mepsPrivate14[, factor])
}

#split
trainidx <- createDataPartition(mepsPrivate14$IPDIS14, p=.8, list = FALSE)
train <- mepsPrivate14[trainidx,vars]
y.test <- mepsPrivate14[-trainidx,target]
x.test <- mepsPrivate14[-trainidx,-which(names(meps) == target)]
